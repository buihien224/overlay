...
def parse_xml(file_path):
    ...
    for child in root:
        if child.tag == 'string':
            ...
            items = {item.attrib['quantity']: item.text for item in child}
...
def find_missing_translations(base_file, translation_file, output_file):
    ...
    missing_plurals = {
        ...
        and not any(k.startswith(excluded_prefixes) for k in value.keys())
        and not any(v.startswith('@') or v.startswith('com.') for v in value.values())
    }
